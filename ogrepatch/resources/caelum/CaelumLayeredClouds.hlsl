...
{
   return float3(
      dot(col, float3(0.299, 0.587, 0.114)),
   ...
   float       cloudCoverageThreshold)
{
   ...
   float aCloud = lerp(
      tex2D(cloud_shape1, finalMassOffset * cloudMassInvScale).r,
   ...
   out     float4 oCol           : COLOR)
{
   ...
   float3 cloudDir = normalize(float3(worldPosition.x, layerHeight, worldPosition.y) - camera_position);
